...
>
> 若走兜底（无 token 编辑模式），请**优先**处理以下低风险、无需实跑验证的工作，避免触碰依赖实时取数的数据逻辑：
> - 优先改 **`DESIGN-TDX-REPLACEMENT.md` §14** 的 P2 设计（数据契约 / 函数签名 / 渲染方案 / 定位边界等纯文档扩展）；
> - 优先改 **`run_report.py` 的结构层**（参数、分支、章节装配顺序、降级分支等），而非直接读取/解析 TDX 返回字段的逻辑；
> - 对必须实跑才能确认的部分（字段 key、阈值、接口可用性），只做「设计 + 占位」，用 `N/A` 或 `# TODO: 用户本地验证` 标注，
>   不要凭记忆猜测 TDX 实际返回结构并据此写死解析代码。
