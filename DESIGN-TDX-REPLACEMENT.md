...
### 12.10 基金报告可借鉴股票报告进一步优化（2026-07-19 分析）

**目的**：对标 `step4_generate_stock_pdf.py`（22 章）与 `step3_generate_pdf_report.py`（当前 8 章）的差异，找出股票报告里**对基金报告有增量价值、且可零 token / 规则化落地**的内容，作为下一轮优化候选。

**章节对照（股票 → 基金现状）**

| 股票报告章节 | 基金报告现状 | 借鉴价值 | 可行性 |
|---|---|---|---|
| §2 情景区间与观察触发器 | 已有（二），但仅文字表 | 中 | — |
| §7 三情景分析（Bull/Base/Bear + 目标价） | 缺失专用情景，仅有配置/观察触发器表 | 高 | 零 token（用指数 PE 25/50/75 分位 × 当前净值推算区间） |
| §8 情景参考区间图 `_trading_plan_chart` | 缺失图表 | 高 | 零 token（PE 分位→净值区间带） |
| §18 未来验证节点（监控清单） | **缺失** | 高 | 零 token（规则化：强化/证伪事件 + 关键时间节点） |
| §19 赚钱机制与商业模式拆解 | **缺失** | 高 | 零 token（被动指数基金机制：跟踪收益−费率−跟踪误差−现金拖累） |
| §21 空方逻辑与风险推演（量化信号 + 黑天鹅） | 仅 4 条泛化风险提示（八） | 高 | 零 token（`etf_analyst_model.risks` 已含信号，需结构化输出） |
| §22 数据来源与取数说明 | **缺失** | 中 | 零 token（列真实 TDX 源 vs 未获取项，回应数据缺口质疑） |
| §6 同行龙头与估值锚对比 | 已有（五）但同类排名未获取 | 高 | 需 fetch：取沪深300/中证500/创业板指 PE 分位 + 同类 ETF 费率 |
| §12 概念板块 / 主营业务构成 | 缺失（=指数行业暴露） | 中 | 需 fetch 成分股权重（TDX 当前无源，标注未来） |
| §9 资金面综合分析（主力净流入） | 六（规模流动性）份额变化未获取 | 中 | 需 fetch share 历史 |
| §10 分红送股历史 | 缺失 | 低 | 需 fetch 分红记录（若 TDX 有） |
| §13 股东结构 → 基金份额持有人结构 | 缺失 | 低 | 需 fetch 持有人结构 |
| §17 反向 DCF 隐含预期 | 不适用（ETF 无盈利） | — | 可改为「指数盈利增速隐含在估值分位中」定性 |

**优先级建议（按价值/成本）**

- **T1（零 token、立即可做，建议本轮回）**：
  1. **监控清单**（对标 §18）：结构化「强化逻辑事件 / 证伪逻辑数据 / 关键时间节点」，全部由现有字段规则生成（估值分位阈值、溢价阈值、规模警戒线、跟踪误差上限、成分股定期调整日）。
  2. **赚钱机制拆解**（对标 §19）：用表格讲清被动指数基金「靠什么赚钱」——跟踪指数收益、费率侵蚀、跟踪误差、现金拖累、复制方式。
  3. **空方逻辑升级**（对标 §21）：把八章 4 条泛化提示升级为「量化看空信号（来自 `risks`）+ 黑天鹅情景」。
  4. **数据来源与取数说明**（对标 §22）：明示真实 TDX 源 vs 未获取项，提升可信度。
  5. **情景参考区间图**（对标 §8 `_trading_plan_chart`）：用指数 PE 25/50/75 分位映射到净值区间带，行业性地补全「二、情景区间」。

- **T2（需补 TDX 数据，已落地 2026-07-20，详见 §12.11）**：
  6. **同类/宽基估值锚对比升级**（对标 §6/§7）：取 000300/000905/399006 的 PE 分位 + 同类 ETF 费率，替换五章「未获取」→ 实际落地在「2.7 宽基估值锚」与「7.1 同类 ETF 费率横向对比」。

- **T3（数据当前无源，标注未来）**：
  7. 指数行业/概念暴露（对标 §12/§9）、份额流变资金面、分红记录、持有人结构。

> 注：T1 全部可在不引入新数据源、不伪造数据的前提下完成；信号与阈值已存在于 `etf_analyst_model.py`，主要是「渲染结构化」与「新增两节 + 一张图」。

---

### 12.11 T2 同类/宽基估值锚对比已落地（2026-07-20）

T2 的两部分均已实现并随 ETF 报告（588000）验证渲染，全程零 token（TDX 落盘，无 LLM 推断）。

**T2a 宽基估值锚对比（对标 §6/§7）**
- 源：`tdx_security_deep_info`（实体类型=指数代码，子工具 `f9_index_historical_pe_pb`），取沪深300(000300)/中证500(000905)/创业板指(399006) 近3年月末 PE·PB·股息率，各 37 点，落盘 `tdx_raw/<code>_index_pe.json`；汇总落盘 `tdx_raw/broad_base_valuation.json`。
- 渲染：报告新增「2.7、宽基估值锚」小节 + 条形图（`_valuation_anchor_chart`），对比各指数 PE 自身近3年历史分位。
- 关键发现：2025–2026 反弹后，沪深300/中证500/创业板指 PE 分位已达 73%/84%/87%（自身历史偏高），而科创50 仍处约 50.9% 分位——绝对 PE 最高，但相对自身历史并不贵。
- 数据质量红线：科创50 整体 PE(TTM) 因成分股亏损失真（TDX 加权口径约 210x），报告沿用市场通用的代表性/中位数 PE≈25.5x（与全文一致），并在 2.7 表注与 JSON `note` 中明确标注失真，未伪造。

**T2b 同类 ETF 费率横向对比**
- 源：`tdx_security_deep_info`（实体类型=基金代码，子工具 `f9_fund_overview_purchase_info` info_type=003 + `f9_fund_diagnosis_basic_info` query_type=jbxx），取 588080/588050/588090/588180/588280 的管理费/托管费/综合费率/基金简称/基金公司，落盘 `tdx_raw/peer_etf_fees.json`（588000 取自 `fin_588000.json` 同源）。
- 渲染：报告「七、费率分析」下新增「7.1、同类 ETF 费率横向对比」小节 + 条形图（`_peer_fee_chart`）。
- 关键发现：588000 综合费率 0.20%/年，与易方达(588080)/华安(588280) 并列同类最低；工银瑞信(588050)、国联安(588180) 为 0.35%，华泰柏瑞(588090) 最高 0.60%。按年复利，0.60% vs 0.20% 持有 10 年成本相差约 4.4 个百分点。
- 设计说明：原 §12.10 写「替换五章『未获取』」，但报告「五、同类基金对比」整节由 `similar_df` 门控（当前无 `similar_funds` 源，整节不渲染），故 T2b 落地在必然渲染的「七、费率分析」下；「五」的完整同类对比（收益/波动/回撤）依赖 `similar_nav` 数据，属 T3 范畴（当前无源，标注未来）。

**落盘文件**：`tdx_raw/broad_base_valuation.json`、`tdx_raw/000300_index_pe.json`、`tdx_raw/000905_index_pe.json`、`tdx_raw/399006_index_pe.json`、`tdx_raw/peer_etf_fees.json`。
**代码改动**：`run_report.py`（加载上述 JSON 进 `data`）、`step3_generate_pdf_report.py`（`_load` 增加 `broad_base_valuation`/`peer_etf_fees` 透传——修复了 `_load` 只按硬编码 key 重建 `d` 导致两者被丢弃的 bug；新增 `_valuation_anchor_chart`/`_peer_fee_chart`；新增 2.7 / 7.1 小节；数据来源 §11 增两行）。
