...
其中 **12 / 13 / 14 / 17 四项已在本文档 §14 展开为正式设计**（含数据契约、函数签名、渲染方案、定位边界与可行性）；15 / 16 为纯规则化数据扩展，待 TDX 接口实测后再补设计。
...
> 落地顺序建议：**12 → 17 → 13 → 14**。前两项纯 A 类、零 token、当轮即可实现；13 需引入可选画像文件；14 依赖编排层多模态，最后做。
