...
## 14. P2 探索项正式设计（12 / 13 / 14 / 17）

> **状态**：本章为设计记录，**尚未实现**。四项均遵守 skill 的核心定位——「研究复盘，不输出买卖建议」。
> 设计中严格区分两类实现：
> - **A 类（CLI 内、零 token、纯规则化）**：可直接进 `step4` / `run_report`，与现有引擎同构。12（轻量版）、13、17 属此类。
...
>
> **合规红线（四项共用）**：不得输出买入/卖出/持有评级、目标价、仓位、止损位、"推荐/回避"等买卖倾向措辞。所有主观推断句一律加 §P1-10 的【判断】/【情景】标签。
...
### 14.6 P2-15 治理与筹码事件时间表（A 类 · 零 token · 数据依赖 TDX 接口 / 公告检索）
...
2. **未来 12 个月筹码事件时间表**：表格 `日期 | 类型 | 明细 | 来源`，按日期升序；无事件则标"未来 12 个月暂无重大筹码事件"。
...
### 14.7 P2-16 扣非净利润 / 周转率 / 存货应收警报（A 类 · 零 token）
...
### 14.5 P2 实施优先级与定位兼容性小结

| 项 | 类别 | Token | 定位兼容性 | 建议优先级 |
|---|---|---|---|---|
| 12 多视角速览（轻量版） | A | 零 | ✅（重量版排除） | **高**：卖点强、零成本 |
| 17 分投资者类型框架 | A | 零 | ⚠️ 需守住"只讲关注点" | **高**：可行性高 |
| 15 治理与筹码时间表 | A | 零（依赖 TDX 接口/公告检索） | ✅ | 中：依赖接口召回率 |
| 16 扣非/周转率警报 | A | 零 | ✅ | 中：依赖明细字段实测 |
| 13 用户画像 | A | 零（建档交互在编排层） | ✅（可选建档） | 中：需文件读取+排序 |
| 14 图文双轨 | B | 编排层非零 / CLI 零 | ✅ 但价值有限 | 低：数据已比图全 |

> 落地顺序建议：**12 → 17 → 15 → 16 → 13 → 14**。前四项（12/17/15/16）纯 A 类、零 token、当轮即可实现——其中 12/17 无额外取数、最快；15/16 需 TDX 重连后实测接口/明细字段再落地。13 需引入可选画像文件；14 依赖编排层多模态，最后做。
